...
Since there's no backend (see "What this is"), a change to how data is computed/displayed doesn't retroactively apply to what's already sitting in a user's `localStorage` — a migration is needed. `run-migrations.ts` is the generic runner: each `Migration` has a permanent `id` (never reuse/rename once shipped) and a `run()` that may be sync or async (returns `void | Promise<void>`); `runMigrations` tracks completed ids in `localStorage["cookerist:completed-migrations"]` so each runs exactly once ever, per browser, persisting that incrementally (after each migration, not batched at the end) so a page closed mid-run doesn't lose credit for ones that did finish. A migration that throws/rejects is still marked completed (logged, never retried) — a broken migration retrying forever is worse than a one-time data gap.
...
Shipped so far: `metric-grocery-units` (converts existing grocery-list items to the always-metric display rule), `categorize-recipe-ingredients` (batches every saved ingredient to Groq once for a category tag + baseName/description correction), `reaggregate-grocery-lists` (re-runs `aggregateGroceryItems` + `carryOverCheckedState` for every existing list so it picks up a recipe correction without a manual edit-and-resave).
...
- **Volume**: same reasoning as mass — always metric, picked by magnitude (ml/l), never a native unit like "cup"/"tbsp". Note this means a small quantity (e.g. 1 tsp + 1 tbsp) still rounds up to at least 100 ml (see Rounding below), same trade-off mass already had for small mg amounts.
- **Length / unrecognized**: largest actually-used unit (in + cm → in) — unaffected by the mass/volume metric-only change above.
